...
int TRYAGAINSONG[][3] = {
  {NOTE_C2, 1}, {NOTE_C2, 1}, {NOTE_C2, 1}, {ENDOFSONG, ENDOFSONG, ENDOFSONG}
};
...
void loop() {
  ...
  while (!done && numTries < maxTries) {                   // set up to loop while playing
    while ( !CircuitPlayground.leftButton() && !CircuitPlayground.rightButton() ) ; // wait to start
    for (int i = 0; i <= gameLevel; ++i) {             // # LEDs show gameLevel
      CircuitPlayground.strip.setPixelColor(i, 255);
    }
    delay(PAUSEB4SEQ);
    ...
    if (playerGuess(gameLevel)) {       // see if person repeated the same sequence
      playCorrectSequence();            // increases level
      done = 1;

      if (gameLevel < MAXLEVEL) {       // if you still have levels to go, increase level by 1
        gameLevel++;
        numTries = 0;                   // resets # tries
      }
    ...
    // IF WRONG
      ...
      numTries++;
      if (numTries < maxTries) {              // if you've used <3 tries
        playTryAgainSequence();               // try again
      }
      else if (numTries == maxTries) {        // if you've used up your 3 tries
        playLoseSequence();                   // you lose
        gameLevel = MINLEVEL;                 // reset to starting level
        numTries = 0;
        done = 1;
      }
    }
...
int playTryAgainSequence() {
  playSong(TRYAGAINSONG);
